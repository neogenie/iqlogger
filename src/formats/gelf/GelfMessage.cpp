...
GelfMessage::gelf::gelf() :
    version{"1.1"}, host{GelfMessage::getHostname()}, timestamp{GelfMessage::getCurrentTime()}, level{1} {}
...
GelfMessage::gelf::gelf(std::string_view message) {
  ...
  if (!ok) {
    ...
    oss << "Coudn't construct GELF message: " << rapidjson::GetParseError_En(ok.Code()) << std::endl
        ...
        << "byte position of error: " << ok.Offset() << std::endl
...
bool GelfMessage::isSystemField(const std::string& key) {
  if (key == "version" || key == "host" || key == "short_message" || key == "full_message" || key == "timestamp" ||
      key == "level") {
    return true;
  }
